 ...
 * decided whether to add exclusion based on B-presses during test.
...
prep_for_analysis <- 
  ...
  mutate(
    ...
    Block = Block - 1) %>%
  select(Condition.Test.OriginalLabel, Condition.Test.Pen, Block, Condition.Test.Audio, ParticipantID, Response.ASHI) %>%
...
d.Exp1a.brm.full <- 
  ...
  Response.ASHI ~
    ...
    (1 + Condition.Test.OriginalLabel * Condition.Test.Pen * mo(Block) * mo(Condition.Test.Audio) | ParticipantID),
  data = 
    d.test %>%
