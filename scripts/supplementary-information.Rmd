...
d.CISP.ALL <-
  ...
  mutate(
    Experiment = factor(paste("CISP", Experiment, sep = "-")),
    across(
      c(ParticipantID, Phase, starts_with("Condition"), starts_with("Item"), Response, Task), 
      as.factor),
    across(
      c(Response.Keycode, Time.StartOfStimulus, Time.EndOfTrial, Response.RT, starts_with("Duration")),
      as.numeric) %>%
    across(
      c(Item.isCatchTrial, Response.CatchTrial),
      as.logical))
