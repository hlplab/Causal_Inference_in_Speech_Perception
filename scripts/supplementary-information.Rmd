  ...
  shared_components <- 
    list(
      ...
      stat_summary(fun = mean, geom = "point", position = position_dodge(dodge), 
                   size = 1),
      stat_summary(fun.data = mean_cl_boot, geom = "linerange", position = position_dodge(dodge), 
                   linetype = 1, alpha = .65),
    ...
    p[[1]] + list(
      stat_summary(
        data = data %>%
          ...
          mutate(Experiment = gsub("CISP-", "Exp ", experiment)), 
        fun = mean, geom = "line", position = position_dodge(dodge + background_dodge), 
        alpha = .5, color = "gray75"),
      stat_summary(
        data = data %>%
          aggregate(Experiment, ParticipantID, Condition.Test.Pen, Condition.Test.Audio, 
                    e = background_experiment) %>% 
          mutate(Experiment = gsub("CISP-", "Exp ", experiment)), 
        fun = mean, geom = "point", position = position_dodge(dodge + background_dodge), 
        size = 1, color = "gray75"),
      stat_summary(
        data = data %>%
          aggregate(Experiment, ParticipantID, Condition.Test.Pen, Condition.Test.Audio, 
                    e = background_experiment) %>% 
          mutate(Experiment = gsub("CISP-", "Exp ", experiment)), 
        fun.data = mean_cl_boot, geom = "linerange", position = position_dodge(dodge + background_dodge), 
        linetype = 1, alpha = .65, color = "gray75"))
