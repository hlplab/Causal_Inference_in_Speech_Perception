...
```{r}
rm(d.Exp3)
load("../data/Experiment-4-before-exclusions.RData") 

d.Exp4 <- 
  d.Exp3 %>%
  filter(Condition.Exposure.Pen == "M") %>%
  mutate(
    Experiment = "CISP-4",
    Condition.Test.Keybindings = NA,                                  # TO DO: delete after unifying data format
    ParticipantID = paste(Experiment, ParticipantID, sep = ".")) %>%
...
d.exposure.Exp4 <- d.Exp4 %>% filter(Phase == "exposure") %>% excludeData()
d.test.Exp4 <- d.Exp4 %>% filter(Phase == "test")
```
