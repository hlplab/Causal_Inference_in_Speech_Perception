...
fit_test_model <- function(data, experiment, audio_only = if ("LJ18-NORM" %in% experiment) T else F) {
  my_formula <- if (audio_only) {
    ...
    bf(paste(
      ...
      if (length(unique(data$Experiment)) > 1) "* Experiment +" else "+",
...
d.test.Exp1 %>% 
  ...
  droplevels() %>%
  data_grid(ParticipantID, Condition.Test.OriginalLabel, Condition.Test.Pen, Condition.Test.Audio, Block = 0) %>%
  add_epred_draws(m.Exp1a, re_formula = NA) %>%
  ggplot(aes(x = Condition.Test.Audio, y = Response.ASHI, color = Condition.Test.Pen)) +
  stat_lineribbon(
    aes(
      y = .epred, 
      alpha = after_stat(level), 
      fill = Condition.Test.Pen),
    .width = c(0.95)) +
  stat_summary(fun = mean, geom = "line", aes(y = .epred)) +
  stat_summary(
    data = 
      d.test.Exp1 %>% 
      filter(Experiment == "CISP-1a") %>%
      group_by(ParticipantID, Condition.Test.Pen, Condition.Test.Audio) %>%
...
```{r results='markup'}
m.Exp2 <- fit_test_model(d.test.Exp2, "CISP-2")
...
bind_rows(d.test.Exp1, d.test.Exp2) %>% 
  ...
  droplevels() %>%
  group_by(Experiment, ParticipantID, Condition.Test.Pen, Condition.Test.OriginalLabel) %>%
  summarise(Response.ASHI = mean(Response.ASHI)) %>%
  ggplot(aes(x = Condition.Test.OriginalLabel, y = Response.ASHI, linetype = Experiment, color = Condition.Test.Pen)) +
...
bind_rows(d.test.Exp1, d.test.Exp2) %>% 
  filter(Experiment %in% c("CISP-1c", "CISP-2")) %>%
  droplevels() %>%
  group_by(Experiment, ParticipantID, Condition.Test.Pen, Condition.Test.Audio) %>%
  summarise(Response.ASHI = mean(Response.ASHI)) %>%
  ggplot(aes(x = Condition.Test.Audio, y = Response.ASHI, color = Condition.Test.Pen, linetype = Experiment)) +
  stat_summary(fun = mean, geom = "line") +
  stat_summary(
    geom = "pointrange",
    position = position_dodge(.25))
...
bind_rows(d.test.Exp1, d.test.Exp2) %>% 
  filter(Experiment %in% c("CISP-1c", "CISP-2")) %>%
  droplevels() %>%
  group_by(Experiment, ParticipantID, Condition.Test.Pen, Condition.Test.Audio, Condition.Test.OriginalLabel) %>%
  summarise(Response.ASHI = mean(Response.ASHI)) %>%
  ggplot(aes(x = Condition.Test.Audio, y = Response.ASHI, color = Condition.Test.Pen, linetype = Experiment)) +
  stat_summary(fun = mean, geom = "line") +
  stat_summary(
