...
plot_data <- function(data, experiment) {
  require(dplyr)
  require(ggplot2)
  require(cowplot)
  ...
  shared_components <- 
    list(
      stat_summary(fun = mean, geom = "line", position = position_dodge(dodge)),
      stat_summary(fun = mean, geom = "point", position = position_dodge(dodge)),
      stat_summary(fun.data = mean_cl_boot, geom = "pointrange", position = position_dodge(dodge), linetype = 1, alpha = .5),
  ...
  cowplot::plot_grid(
    ...
    rel_widths = c(.3, .7), rel_heights = 1.5)
}
```

```{r fig.height=8, fig.width=6}
ggsave(
  plot_data(d.test.Exp1, experiment = c("CISP-1a", "CISP-1b", "CISP-1c")),
...
## Data import
We import and format the data for Experiments 2.
