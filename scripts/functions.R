...
formatData <- function(.data, experiment, exclude_based_on_catch_trials = T) {
  ...
  .data %<>%
    ...
    add_exclusion_information(exclude_based_on_catch_trials = exclude_based_on_catch_trials) %>%
...
format_hypothesis_tables <- function(table, experiment, BF.max = 4000) {
  table %>%
  rename(
    `$\\hat{\\beta}$` = Estimate, BF = Evid.Ratio, SE = Est.Error, `$p_{posterior}$` = Post.Prob, ` ` = Star,
    `CI$_{l}$` = CI.Lower, `CI$_{u}$` = CI.Upper) %>%
  mutate(
    Exp = gsub("(CISP|Exp)", "", experiment),
    across(
...
plot_data <- function(data, experiment, background_experiment = NULL) {
  ...
  exposure_experiment <- length(unique(data[data$Experiment %in% experiment,]$Condition.Exposure.LexicalLabel)) > 1
