...
d.Exp2a <- d.test.D %>%
  mutate(Experiment = "CISP-2a")

load("../data/Experiment-NORM-E-before-exclusions.RData") #named d.test.E
d.Exp2b <- d.test.E %>%
  mutate(Experiment = "CISP-2b",
         # One participant responded "Female" in answer to Participant.Age??
         # should be tracked down at some point...
         Participant.Age = as.double(Participant.Age))


d.Exp1and2 <- bind_rows(d.Exp1a, d.Exp1b, d.Exp1c,
                      d.Exp2a, d.Exp2b) %>%
...
load("../data/Experiment-3-before-exclusions.RData") #named d.Exp2
# can't be renamed to d.Exp3 because the load of Exp4 would overwrite
CISP.Exp3 <- d.Exp2 %>%
  rename(Participant.AudioType = audio_type,
         Talker.Sex = sex) %>%
  mutate(Experiment = "CISP-3")

load("../data/Experiment-4-before-exclusions.RData") #named d.Exp3
d.Exp4 <- d.Exp3 %>%
  rename(Participant.AudioType = audio_type,
         Talker.Sex = sex) %>%
  mutate(Experiment = "CISP-4")
...
rm(d.test.A, d.test.B, d.test.C, d.test.D, d.test.E,
   d.Exp1a, d.Exp1b, d.Exp1c, d.Exp2a, d.Exp2b,
   d.Exp2, CISP.Exp3)
```
