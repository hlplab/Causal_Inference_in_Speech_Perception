...
d.CISP.ALL <-
  ...
  # Add one empty participant to Experiment 1c to reflect the one we lost due to technical issues
  bind_rows(tibble(Experiment = "CISP-1c", ParticipantID = "1c.64", Exclude_Participant.Reason = "Technical difficulty")) %>%
  mutate(
    across(
...
d.test <- 
  bind_rows(d.test.Exp1, d.LJ18.test) %>%
  add_exclusion_information()
```
...
(ref:exclusions-exp1) Mean and SD of log-transformed reaction times for participants in Experiments 1a-c, along with reason the participant was excluded from analysis. One additional participant in Experiment 1c was excluded due to technical issues (not shown since we do not have RTs for that participant).
...
# how many pt's worth of data do we have before exclusions?
# d.test %>%
...
(ref:exclusions-exp1-RTs) Trial exclusions based on RTs in Experiment 1a-c. Log-transformed RTs were first scaled within participants. Since participants' RTs decreased throughout the experiment (black line), we excluded responses that where more than 3 SDs faster or slower than the scaled log-RTs across participants *at that trial position* (gray ribbon). This exclusion was applied *after* removing excluded participants.

```{r exclusions-exp1-RTs, fig.width=3*fig.base_width, fig.height=fig.base_height*1.25, fig.cap="(ref:exclusions-exp1-RTs)", results='asis', out.width="100%"}
d.test %>%
  excludeParticipants() %>%
  filter(Experiment != "LJ18-NORM") %>%
  group_by(Experiment, ParticipantID) %>%
  # Get mean of log-transformed RTs for each participant
  mutate(
  ...
  mutate(
    meanRibbon = mean(Response.log_RT_scaled, na.rm = T),
    upperRibbon = meanRibbon + 3 * sd(Response.log_RT_scaled, na.rm = T),
    lowerRibbon = meanRibbon - 3 * sd(Response.log_RT_scaled, na.rm = T)) %>%
  ggplot(aes(x = Trial)) +
  geom_ribbon((aes(ymin = lowerRibbon, ymax = upperRibbon)), alpha = 0.2, fill = "black") +
  geom_point(aes(y = Response.log_RT_scaled, color = Exclude_Trial.because_of_RT), size = .25, alpha = .5) +
  geom_line((aes(y = meanRibbon)), color = "black") +
  scale_y_continuous("Scaled log-transformed RTs") +
  scale_color_discrete("Trial excluded", breaks = c(T, F), labels = c("yes", "no")) +
...
# mean(d.test %>% 
#        mutate(Response.log_RT = log10(Response.RT)) %>%
#        filter(Experiment %in% c("CISP-1a", "CISP-1b", "CISP-1c")) %>% pull(Response.log_RT))
# 
# sd(d.test %>% 
#        mutate(Response.log_RT = log10(Response.RT)) %>%
#        filter(Experiment %in% c("CISP-1a", "CISP-1b", "CISP-1c")) %>% pull(Response.log_RT))
# 
# d.test %>%
#   filter(Exclude_Trial.because_of_RT == TRUE | is.na(Response.ASHI)) %>%
#   mutate(reason = case_when(
#     Exclude_Trial.because_of_RT == TRUE ~ "RT",
#     is.na(Response.ASHI) ~ "Catch",
#     Exclude_Trial.because_of_RT == TRUE & is.na(Response.ASHI) ~ "Both"
#   )) %>%
#   group_by(Experiment, reason) %>%
#   summarise(count = n())
```
...
The next set of questions assessed how participants perceived the speech of the talker, including potential effects of the pen. The first question simply asked participants whether they noticed anything odd about the talker, followed by an open-ended text prompt. We annotated participants' text responses. After agreeing on the criteria, annotations were completed by one of the authors (GEK) and spot-checked by another author (TFJ). The complete set of annotations is available as part of the OSF repo under data/CISP_data_participant-comments.csv. The file contains participants' full comment and 10 binary annotation columns:

  * Whether the participant's comment described the **talker's speech in the experiment as in anyway unusual** (e.g., "accented", exhibiting a "lisp").
  * Whether the participant's comment mentioned the **pen**.
  * Whether the participant's comment mentioned that the **pen in the mouth made it difficult to determine what the speaker said**.
  * Whether the participant's comment mentioned that the **pen in the mouth affects pronunciation**.
  * Whether the participant's comment mentioned that the **pen was perhaps the purpose of the experiment**.
 
  * Whether the participant's comment mentioned **the white dot** (which was never present, which made some participants nervous that they would have missed the catch trials, and many of these comments asserted that the participant was, in fact, very attentive and/or that they sometimes pressed SPACE---indicating a catch trial---even though they didn't see the white dot, simply because they thought if they didn't it would mean we would consider them inattentive).
 
  * Whether the participant's comment mentioned **audio or video playback issues**.
  * Whether the participant's comment mentioned **misalignment of the audio and video**.
 
  * Whether the participant's comment sounded **non-native**. (added after feedback from reviewers)
  * Whether the participant's comment sounded **bot-like**.  (added after feedback from reviewers)

```{r}
d.comment <- read.csv("../data/CISP_data_participant-comments.csv")

annotation_numbers <- function(var, subset = unique(d.comment$Experiment)) {
  df <- d.comment %>%
    filter(Experiment %in% subset)
  paste0(x <- sum(df[[paste0("Talker.SpeechDescription_", var)]] == "yes"), " (", round(x / nrow(df) * 100, 1), "%)")
}
```

While only `r annotation_numbers("Odd")` participants across the three experiments considered the talkers' speech atypical (e.g., "accented", exhibiting a "lisp"),  `r annotation_numbers("Pen")` participants commented on the fact that the talker often had a pen in the mouth. The latter is expected given that the pen was in the talker's mouth on half of the trials, and thus rather salient. Of the responses that mentioned the pen, some claimed that the pen affected the pronunciation of the talker (in reality the audio recording was by design independent of the location of the pen). Most participants simply noticed the pen. Some participants, however, further noted that the pen affected their ability to tell whether the talker was saying *asi* or *ashi* (`r annotation_numbers("PenMadeCategorizationDifficult")`), or that the pen affected the talker's pronunciation (`r annotation_numbers("PenAffectsPronunciation")`), and some hypothesized that the experiment was about the pen (`r annotation_numbers("PenHypothesizedToMatter")`). While these responses suggests that relatively few participants had conscious hypotheses about the effects of the pen, Section \@ref(sec:subset-analyses) below presents subset analyses of Experiments 1a-c that exclude those participants. 

A substantial minority of participants---`r annotation_numbers("AudioVideoMisaligned")`---noted misalignment between the sound and the video. This is an inevitable consequence of the approach we chose to investigate the effect of the pen (see main text for our rationale for this approach). Critically thought, it is unclear how awareness of audiovisual misalignment would bias the results we reliably observed across Experiments 1a-c. Finally, only a very small percentage of participants were annotated as potentially non-native speakers (`r annotation_numbers("PotentialNonNative")`) or potentially a bot (`r annotation_numbers("PotentialBot")`). Given the highly subjective nature of this annotation, and our own uncertainty about it (based on a single free-form text response), we did not further consider this annotation.^[Given that the R markdown code to this SI as well as the data---including our annotations---are shared on OSF, other researchers can explore the consequences of different exclusion criteria with just a few lines of code changes.]
...
(ref:results-exp1) Summary of participants’ responses in Experiments 1a-c, depending on pen location and acoustic continuum step (Panel A) or visual bias (Panel B). For comparison, responses from a audio-only experiment using some of the same stimuli [@liu-jaeger2018] are shown in gray. Comparison to the audio-only (and thus no pen) continuum used in all previous work helps to assess whether a audiovisual stimulus with the pen in the hand is actually identical to an audio-only stimulus (an assumption made in some previous work). While presented here in case of interest, the data from [@liu-jaeger2018] are omitted for clarity from the figure presented in main text.
...
```{r results-exp1-main-text, fig.width=fig.base_width*2, fig.height=fig.base_height*3, out.width="80%"}
p <-
  plot_data(
    d.test,
    experiment = c("CISP-1a", "CISP-1b", "CISP-1c"))
...
ggsave(
  ...
  width = fig.base_width * 2,
  height = fig.base_height * 2 + .25)
```
...
As noted in the description of the survey results in Section \@ref(sec:survey-results), some participants commented on potential effects of the pen. We thus conducted a subset analysis to addresses the potential that those participants might have responded in a way that reflects conscious deliberations about the effect of the pen, rather than the presumably automatic, subconscious processes entailed by compensation. For this analysis, we excluded all participants who commented that 1) the pen made it difficult to see what the talker said, 2) the pen affected pronunciation, or 3) the experiment was about the effects of the pen. This removed `r  (d.test.Exp1 %>% distinct(ParticipantID) %>% nrow()) - (d.test.Exp1 %>% 
    ...
    filter(
      Talker.SpeechDescription_PenMadeCategorizationDifficult != "yes",
      Talker.SpeechDescription_PenAffectsPronunciation != "yes",
      Talker.SpeechDescription_PenHypothesizedToMatter != "yes") %>% distinct(ParticipantID) %>% nrow()` participants for analysis. As reported in the following subsections, all results reported in the main analyses held for this much smaller subset of participants. In fact, the critical effect estimates ($\hat{\beta}$) for the subset analyses were as large or larger than in the main analyses.
...
d.test.Exp2.old <- d.test.Exp2
d.test.Exp2 %<>% 
  run_exclusions(c("CISP-2", "CISP-2b"))

# mean(d.test.Exp2 %>%
#        mutate(Response.log_RT = log10(Response.RT)) %>%
#        filter(Experiment == "CISP-2") %>%
#        pull(Response.log_RT))
# 
# sd(d.test.Exp2 %>%
#        mutate(Response.log_RT = log10(Response.RT)) %>%
...
(ref:exclusions-exp2-RTs) Trial exclusions based on RTs in Experiment 2 and 2b. Log-transformed RTs were first scaled within participants. Since participants' RTs decreased throughout the experiment (black line), we excluded responses that where more than 3 SDs faster or slower than the scaled log-RTs across participants *at that trial position* (gray ribbon). This exclusion was applied *after* removing excluded participants.

```{r exclusions-exp2-RTs, fig.width=2*fig.base_width, fig.height=fig.base_height*1.25, fig.cap="(ref:exclusions-exp2-RTs)", results='asis', out.width="66%"}
d.test.Exp2.old %>%
  excludeParticipants() %>%
  group_by(Experiment, ParticipantID) %>%
  # Get mean of log-transformed RTs for each participant
...
To inform the potential role of the black box in distracting listeners from the pen, we examined the degree to which participants mentioned the pen in the free-response text box of the survey. We thus compared Experiment 2 to 1a-c (of which 1c was the most similar to Experiment 2). Mentions of the pen *de*creased somewhat from `r annotation_numbers("Pen", subset = "CISP-1c")` in Experiment 1c to `r annotation_numbers("Pen", subset = "CISP-2a")` in Experiment 2. 

However, pen-mention also differed across Experiments 1a-c. A logistic regression comparing the probability of pen-mention across Experiments 1a-c and 2 did not find a significant difference (p > .3). Even when Experiment 2 was compared to only Experiment 1c, the difference between the experiments was not significant (p > .2).

```{r, fig.width=fig.base_width * 2, fig.height=fig.base_height*1.25, out.width="50%"}
d.pen_mention <-
  d.comment %>%
  pivot_longer(
    cols = starts_with("Talker.SpeechDescription_"),
    ...
    values_to = "response") %>%
  filter(
    question == "Talker.SpeechDescription_Pen",
    Experiment %in% c("CISP-1a", "CISP-1b", "CISP-1c", "CISP-2a")) %>%
  mutate(
    Experiment = gsub("CISP-", "", Experiment),
    Experiment = factor(ifelse(Experiment == "2a", "2", Experiment)),
    response = factor(ifelse(response == "yes", "yes", "no")),
    question = "Was the pen mentioned?") 

d.pen_mention %>%
  ggplot(aes(x = Experiment, fill = response)) +
  geom_bar(position = "fill", aes(group = response)) +
  xlab("Experiment")+ 
  scale_y_continuous("Percent responses", labels = scales::percent) +
  scale_fill_grey("Did the participant mention the pen?") 

# glm(
#   response == "yes" ~ 1 + ExperimentGroup,
#   data = 
#     d.pen_mention %>%
#     mutate(ExperimentGroup = factor(ifelse(Experiment == "2", 1, -1))),
#   family = binomial(link = "logit")) %>%
#   summary()

# glm(
#   response == "yes" ~ 1 + Experiment,
#   data =
#     d.pen_mention %>%
#     filter(Experiment %in% c("1a", "2")),
#   family = binomial(link = "logit")) %>%
#   summary()
```
...
```{r, results='asis'}
m.Exp2 <- fit_test_model(data = d.test.Exp2, experiment = "CISP-2")
m.Exp2 %>%
  brms_SummaryTable() %>%
...
m.Exp2vsExp1c <-
  fit_test_model(
    ...
    # Since contrasts are already defined don't run prep_for_analysis
    .prep_for_analysis = F)
```

Overall, the results of this analysis do not provide any notable evidence for the occlusion hypothesis while providing strong evidence for the compensation hypothesis. In the middle of the acoustic continuum, *ashi*-responses were *more* likely in Experiment 2, compared to even the pen-in-hand condition of Experiment 1c (BF = 2 = 1 / .5). This is the opposite of what would be expected if the main effects of pen location in Experiments 1a-c were due to visual occlusion. There also was strong evidence that the pen-in-mouth condition of Experiment 1c resulted in fewer *ashi*-responses than in Experiment 2 (BF = 29.5).
...
hypothesis(
  ...
  kable(
    format = "latex",
    caption = "Effects of pen location: occlusion vs. hand",
    align = c(rep("l", 2), rep("r", 6), "l"),
    escape = F) %>%
  column_spec(2, width = "7cm") %>%
  column_spec(7, width = "1.1cm") %>%
...
hypothesis(
  ...
  kable(
    format = "latex",
    caption = "Effects of pen location: mouth vs. occlusion",
    align = c(rep("l", 2), rep("r", 6), "l"),
    escape = F) %>%
  column_spec(2, width = "7cm") %>%
  column_spec(7, width = "1.1cm") %>%
...
```{r, results='asis'}
m.Exp2b <- fit_test_model(data = d.test.Exp2, experiment = "CISP-2b")
m.Exp2b %>%
  brms_SummaryTable() %>%
