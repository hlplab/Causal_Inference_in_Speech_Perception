...
read.csv("../data/CISP_data_participant-comments.csv") %>%
  ...
  geom_bar() +
  facet_wrap(question ~ Experiment, ncol = 3) +
  scale_x_continuous("Response",
                   breaks = c(0, 1),
                   labels = c("no", "yes"))

# d.Exp1.survey %>%
#   select(Experiment, ParticipantID, Talker.SpeechDescription) %>%
#   mutate(
#     Talker.SpeechDescription_Odd = NA,
#     Talker.SpeechDescription_AudioVideoMisaligned = NA,
#     Talker.SpeechDescription_Pen = ifelse(str_detect(Talker.SpeechDescription, regex("pen", ignore_case = T)), "yes", NA),
#     Talker.SpeechDescription_WhiteDot = ifelse(str_detect(Talker.SpeechDescription, regex("white dot", ignore_case = T)), "yes", NA),
#     Talker.SpeechDescription_NotOdd = 
#       case_when(
#         is.na(Talker.SpeechDescription) ~ "yes", 
#         Talker.SpeechDescription %in% c("I did not.", "N/A", "NOTHING", "i dont think so") ~ "yes", 
#         str_detect(Talker.SpeechDescription, "^(no|No)") ~ "yes", 
#         str_detect(Talker.SpeechDescription, regex("I did not notice anything odd", ignore_case = T)) ~ "yes",
#         str_detect(Talker.SpeechDescription, "The speaker did not seem odd to me")  ~ "yes",
#         T ~ NA_character_)) %>%
#   write_csv(file = "../data/CISP_data_participant-comments.csv", na = "") %>%
