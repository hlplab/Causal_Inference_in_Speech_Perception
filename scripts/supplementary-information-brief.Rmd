...
Extra analyses of trial-level RT
maybe keep this sort of plot?
```{r}
d.test %>%
    group_by(Experiment, ParticipantID) %>%
  ...
  mutate(upperRibbon = mean(Response.log_RT_scaled) + 3 * sd(scale(Response.log_RT_scaled)),
         lowerRibbon = mean(Response.log_RT_scaled) - 3 * sd(scale(Response.log_RT_scaled))) %>%
  ggplot(aes(x = Trial)) +
  geom_point(aes(y = Response.log_RT_scaled,
             color = Exclude_Trial.because_of_RT)) +
    geom_line(aes(y = upperRibbon)) +
  geom_line(aes(y = lowerRibbon)) +
