...
To inform the potential role of the black box in distracting listeners from the pen, we examined the degree to which participants mentioned the pen in the free-response textbox of the survey. We thus compared Experiment 2 to 1c. If anything, mentions of the pen did *in*crease somewhat from `r annotation_numbers("Pen", subset = "CISP-1c")` in Experiment 1c to `r annotation_numbers("Pen", subset = "CISP-2a")` in Experiment 2. 
...
d.comment %>%
  pivot_longer(
    ...
    names_to = "question",
    values_to = "response") %>%
  filter(question == "Talker.SpeechDescription_Pen",
         Experiment %in% c("CISP-1a", "CISP-2a")) %>%
  mutate(
    Experiment_group = ifelse(Experiment == "CISP-2a", "2", "1c"),
    response = ifelse(response == "yes", "yes", "no"),
  ...
  scale_fill_discrete("Pen mentioned?")
```
