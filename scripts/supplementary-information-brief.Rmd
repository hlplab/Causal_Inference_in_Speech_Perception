...
d.comment %>%
  ...
  scale_x_continuous("Response",
                   ...
                   labels = c("no", "yes"))
```
To inform the potential role of the black box in distracting listeners from the pen, we examine the degree to which mentions of the pen changed between experiments 1a-c and 2a. Mentions did indeed decrease from `r annotation_numbers("Pen", subset = c("CISP-1a", "CISP-1b", "CISP-1c"))` to `r annotation_numbers("Pen", subset = "CISP-2a")`. However, given the low overall frequency of pen mentions in this free response question, this change cannot be strongly interpreted. 
```{r, fig.width=fig.base_width*3, fig.height=fig.base_height*4, out.width="66%"}
d.comment %>%
  pivot_longer(
    cols = starts_with("Talker.SpeechDescription_"),
  ...
  filter(question == "Talker.SpeechDescription_Pen",
         Experiment != "CISP-2b") %>%
  mutate(
    Experiment_group = ifelse(Experiment == "CISP-2a", "2", "1a-c"),
...
While presented here in case of interest, the data from [@liu-jaeger2018] are omitted for clarity from the figure presented in main text.
