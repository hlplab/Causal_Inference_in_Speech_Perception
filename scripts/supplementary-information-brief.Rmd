...
### Survey results
All survey responses are shared as part of the OSF repo for this project. The clear majority of all participants did not report any issues with the audio or video recordings.
...
The clear majority of participants did not consider the speech they heard to be unusual, until subsequent questions explicitly pointed to the pronunciation of "s" and "sh". The first question that assesses this simply asked participants whether they noticed anything odd about the talker, followed by an open-ended text field. We categorized participants' answers in terms of whether their statement mentioned that 1) the talker was described as sounding odd (e.g., having a lisp), 2) the talker had a pen in the mouth, 3) the pen affected the talker's speech, and 4) the audio and video were misaligned. While very few participants considered the talkers' speech atypical (e.g., "accented", exhibiting a "lisp"), many participants commented on the fact that the talker often had a pen in the mouth. Of these, some claimed that the pen affected the pronunciation of the talker (in reality the audio recording was by design independent of the location of the pen). Finally, about 15-20% of participants noted misalignments between the sound and the video. 

```{r, fig.width=fig.base_width*3, fig.height=fig.base_height*4, out.width="66%"}
read.csv("../data/CISP_data_participant-comments.csv") %>%
  pivot_longer(
  ...
  filter(
    Experiment != "", 
    question %in% c("Was the talker described as sounding atypical?", "Was the pen mentioned?", "Did the pen described as affecting pronunciation?", "Was audiovisual misalignment mentioned?")) %>%
  mutate(question = factor(question, levels = c("Was the talker described as sounding atypical?", "Was the pen mentioned?", "Did the pen described as affecting pronunciation?", "Was audiovisual misalignment mentioned?"))) %>%
...
<!-- ## Subset analyses -->
<!-- Since some participants commented on the pen affecting the pronunciation of the talker, we conducted a subset analysis that excluded those participants. This addresses the potential that those participants might have responded in a way that reflects conscious deliberations about the effect of the pen, rather than the presumably automatic, subsconscious processes entailed by compensation.  -->
...
<!--   data = d.test.Exp1 %>% filter(Talker.SpeechDescription_PenAffectsPronunciation != "yes"),  -->
<!--   experiment = "CISP-1a", -->
<!--   file = paste("../models/Exp", paste(experiment, collapse = "-"), "wo participants who commented on pen affecting pronunciation", sep = "-")) -->
