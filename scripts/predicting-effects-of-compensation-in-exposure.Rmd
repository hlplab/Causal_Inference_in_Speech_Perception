...
```{r}
d.exposure %>%
  ggplot(aes(x = Trial, y = Condition.Exposure.Label, fill = Item.Type, alpha = Item.Pen)) +
  geom_point() +
...
d.exp %>%
  ...
  facet_grid(Condition.Exposure.Label ~ Item.VisualLabel)
...
d.exp  %>%
  ...
  geom_point() +
  facet_grid(Item.Pen + Condition.Exposure.Label ~ Item.VisualLabel)
...
posterior_s_bias.PiH <-
  update_NIW_ideal_adaptor_incrementally(
    ...
    exposure =
      ...
      filter(
        Condition.Exposure.Label == "S-bias",
        Item.Pen == "H"),
...
posterior_sh_bias.PiH <-
  update_NIW_ideal_adaptor_incrementally(
    ...
    exposure =
      ...
      mutate(cog = ifelse(segment.perceived_as_intended == "no", NA, cog)) %>%
      filter(
        Condition.Exposure.Label == "SH-bias",
        Item.Pen == "H"),
    exposure.category = "Item.VisualLabel",
    exposure.cues = "cog")
