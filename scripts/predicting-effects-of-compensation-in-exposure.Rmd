...
devtools::install_github("hlplab/MVBeliefUpdatr", ref = "path-stanfit-plotting")
...
```{r load-data, message=FALSE, results='asis'}
...
d.perception <-
  ...
  mutate(
    ...
    # Item.VisualLabel for the sake of coherence.
    Item.VisualLabel = 
      case_when(
...
d.exposure <-
  ...
  left_join(
    d.acoustics %>%
      ...
      rename(Item.VisualLabel = segment) %>%
      # scale cog based on statistics used for fitted model
      mutate(cog_gs = (cog - stats.cog$mean) / (2 * stats.cog$sd)) %>%
      select(word, type, cog, cog_gs, Item.VisualLabel),
    by = 
      join_by(
        Item.VisualLabel == Item.VisualLabel, 
        Item.Word == word, 
        Item.Type == type))
```
...
d.acoustics %>%
  ...
  scale_fill_manual("Category",
                    values = c(color.s, color.sh, color.s_sh)) +
  facet_wrap(~ type)
```
...
```{r, fig.height=fig.base_height*10, fig.width=fig.base_width*5}
d.exposure %>%
  # Select one instance of each combination of Experiment and exposure conditions
  group_by(Experiment, Condition.Exposure.LexicalLabel, Condition.Exposure.Pen) %>%
  filter(ParticipantID == first(ParticipantID)) %>%
  ggplot(
    aes(
      x = Trial, 
      y = paste0(Condition.Exposure.LexicalLabel, "-bias"),
      fill = 
        factor(
          case_when(
            Item.Type == "typical" ~ Item.VisualLabel, 
            Item.Type == "shifted" ~ "?",
      ...
      alpha = Item.Pen)) +
  geom_tile() +
  scale_y_discrete("Lexical bias") +
  scale_fill_manual(
    "Item type",
    values = c(color.s, color.sh, color.s_sh, color.filler)) +
  scale_alpha_manual(
    breaks = c("M", "H"),
    values = c(0.5, 1)) +
  facet_grid(Experiment + Condition.Exposure.Pen ~ .) +
  coord_cartesian(expand = F)
```
### Catch trial accuracy during exposure and text
We need to exclude participants based on their catch trial performance, following the same exclusion criteria applied to the test-only experiments.

```{r, fig.width=fig.base_width*8, fig.height=fig.base_height*2+.5, warnings=FALSE, fig.cap=c("Proportion correct catch answers in lexical decision task."), message=FALSE}
p <-
  d.exposure %>%
  bind_rows(d.test) %>%
  # One participant for some reason has *one* trial for which Item.isCatchTrial is NA during test
  # excluding this for now:
  filter(!is.na(Item.isCatchTrial)) %>%
  group_by(Experiment, Phase, Item.Pen, Item.isCatchTrial, ParticipantID) %>%
  summarise(Accuracy.bySubject = mean(Response.CorrectCatch, na.rm = T)) %>%
  ggplot(
    aes(
      x = Item.Pen, 
      y = Accuracy.bySubject, 
      color = Item.isCatchTrial, 
      fill = Item.isCatchTrial,
      shape = Item.Pen)) +
  geom_dotplot(
    binaxis = "y", 
    stackdir = "center", 
    ...
    binwidth = .02,
    dotsize = .6, alpha = .2, position = position_dodge(.5)) +
  stat_summary(
    fun = mean, geom = "point", 
    position = position_dodge2(.5)) +
  stat_summary(
    fun.data = mean_cl_boot, geom = "linerange", 
    position = position_dodge2(.5), alpha = .7) +
  scale_x_discrete("") +
  scale_y_continuous("Proportion correct responses") +
  scale_color_manual("Is catch trial",
                    breaks = c(T, F),
                    labels = c("yes", "no"),
                    values = c("black", "gray"),
  ...
  facet_grid(Phase ~ paste(Experiment), space = "free_x", scales = "free_x")
...
```{r, fig.width=fig.base_width*5, fig.height=fig.base_height+.5, warnings=FALSE, fig.cap=c("Proportion correct answers in lexical decision task for critical and filler words with typical pronunciation.", 'Proportion "word" answers in lexical decision task for critical words with shifted pronunciation.'), message=FALSE}
p <- 
  p %+%
  (d.exposure %>%
  filter(Item.Type != "shifted", !Item.isCatchTrial) %>%
  # Make sure catch answers are treated as wrong for all the non-catch trials
  mutate(Response = ifelse(is.na(Response), "WRONG", as.character(Response))) %>%
  group_by(Experiment, Item.Pen, Condition.Exposure.LexicalLabel, ParticipantID) %>%
...
```{r, fig.width=fig.base_width*6, fig.height=fig.base_height*2+1, fig.cap=c("Effects of exposure conditions on categorization responses during test", "Same but by test block.")}
d.test %>%
  filter(
    Experiment %in% unique(d.exposure$Experiment),
    # Some participants give catch trial responses during test (even through test
    # never contain catch trials)
    !Response.CatchTrial) %>%
  ggplot(
    aes(
      x = cog,
...
m1 <-
  brm(
    formula =
      # Using Item.Pen instead of Condition.Test.Pen to make things easier below
      # (but the two variables are identical for the test phase and thus for the
      # experiments fit here). 
      # Same for Item.VisualLabel instead of Condition.Test.OriginalLabel.
      Response.ASHI ~ 1 + Experiment * Item.Pen * Item.VisualLabel * cog_gs +
      ...
      # (presumable incl. cog).
      (1 + Item.Pen * Item.VisualLabel * cog_gs | ParticipantID),
    data =
      d.test.for_compensation %>%
      prep_for_analysis(),
    family = "bernoulli",
...
NOTE: consider sampling instead such that the chance of being interpreted as /s/ vs /sh/ changes across continuum, rather than a strict cutoff.
...
d.exp <-
  ...
  # data anyway)
  group_by(word, type, Item.VisualLabel, Item.Pen, cog, cog_gs) %>%
  summarise(predicted_probability.SH = mean(Estimate)) %>%
  mutate(
    Condition.Exposure.LexicalLabel =
      case_when(
        Item.VisualLabel == "S" & type == "typical" ~ "SH-bias",
        Item.VisualLabel == "S" & type == "shifted" ~ "S-bias",
        Item.VisualLabel == "SH" & type == "typical" ~ "S-bias",
        Item.VisualLabel == "SH" & type == "shifted" ~ "SH-bias"),
  ...
  # perceived as intended.
    ...
    segment.perceived_as_intended =
      case_when(
        Item.VisualLabel == "S" & 1 - predicted_probability.SH < .data$cutoff_s ~ "no",
        Item.VisualLabel == "S" & 1 - predicted_probability.SH >= .data$cutoff_s ~ "yes",
        Item.VisualLabel == "SH" & predicted_probability.SH < .data$cutoff_sh ~ "no",
        Item.VisualLabel == "SH" & predicted_probability.SH >= .data$cutoff_sh ~ "yes"))
```
...
if (RESET_MODELS | !file.exists("../data/sample_data_compensated.RData")) {
  sampledata <- 
    ...
    group_by(Item.VisualLabel, Item.Pen, cog) %>%
  ...
  for (label in unique(sampledata$Item.VisualLabel)) {
    temp_no_pen <-
      sampledata %>%
      filter(Item.Pen == "H", Item.VisualLabel == label)
    
    temp_pen <-
      sampledata %>%
      filter(Item.Pen == "M", Item.VisualLabel == label)
...
add_compensated_cue <- 
  . %>%
    left_join(
      sampledata_pen %>%
        select(Item.VisualLabel, cog, closest_no_pen_cog),
      by = c("Item.VisualLabel", "cog")) %>%
    { if (!("Item.Pen" %in% names(.))) { mutate(., Condition.Pen = "H") } else . } %>%
    mutate(
      cog_compensated = ifelse(Item.Pen == "M", closest_no_pen_cog, cog),
...
d.exp.compensated <- 
  ...
  mutate(
    ...
    segment.perceived_as_intended =
      case_when(
        Item.VisualLabel == "S" & 1 - predicted_probability.SH < cutoff_s ~ "no",
        Item.VisualLabel == "S" & 1 - predicted_probability.SH >= cutoff_s ~ "yes",
        Item.VisualLabel == "SH" & predicted_probability.SH < cutoff_sh ~ "no",
        Item.VisualLabel == "SH" & predicted_probability.SH >= cutoff_sh ~ "yes"))
```
...
d.exp %>%
  ...
  facet_grid(Condition.Exposure.LexicalLabel ~ Item.VisualLabel)
...
d.exp  %>%
  ...
  geom_point() +
  facet_grid(Item.Pen + Condition.Exposure.LexicalLabel ~ Item.VisualLabel)
...
IO <-
  ...
  data = d.exp %>% filter(Item.Pen == "H", type == "typical"),
  category = "Item.VisualLabel",
  cues = "cog")
...
posterior_s_bias.PiH <-
  update_NIW_ideal_adaptor_incrementally(
    ...
    exposure =
      ...
      filter(
        Condition.Exposure.LexicalLabel == "S-bias",
        Item.Pen == "H"),
    exposure.category = "Item.VisualLabel",
    exposure.cues = "cog")
...
posterior_sh_bias.PiH <-
  update_NIW_ideal_adaptor_incrementally(
    ...
    exposure =
      ...
      mutate(cog = ifelse(segment.perceived_as_intended == "no", NA, cog)) %>%
      filter(
        Condition.Exposure.LexicalLabel == "SH-bias",
        Item.Pen == "H"),
    exposure.category = "Item.VisualLabel",
    exposure.cues = "cog")
...
if (!INCLUDE_ANIMATIONS & !file.exists("../figures/updating.webm")) {
  a <- 
  plot_expected_categorization_function_1D(
      summary %>% 
...
# Confirm that only Items with PIM are included
# d.test.for_ibbu %>% group_by(Item.Pen) %>% tally()
```

### Ignoring pen (without modeling maximal acceptable shift for "word"-responses during exposure)
This approach assumes that all inputs during exposure are labeled as intended by the experimenter.
...
m.pen_ignored <-
  fit_NIW_ideal_adaptor(
    staninput = make_staninput(
      ...
      response = "Response", 
      center.observations = T, scale.observations = F, tau_scale = sd(d.exposure.for_ibbu$cog),
      model_type = "NIW_ideal_adaptor"),
    warmup = 2000, iter = 3000, chains = 4, control = list(adapt_delta = .99), silent = 2,
...
```{r, fig.height=fig.base_height*2+1, fig.width=fig.base_width*2, fig.cap=c("Predicted categorization function for Experiment 3", "Predicted categorization function for Experiment 4", "Predicted categorization function for Experiment 5"), message=FALSE}
map(
  .x = list(3, 4, 5),
  .f = function(exp) {
    ...
    plot_expected_categorization(
      ...
      stat_summary(
        data = 
          d.test.for_ibbu %>% 
          filter(Experiment == paste0("CISP-", exp)) %>%
          mutate(
            group = Condition,
            token.cues = factor(signif(cog), levels = sort(unique(signif(cog)), decreasing = T))) %>%
...
### Compensating for pen (without modeling maximal acceptable shift for "word"-responses during exposure)
This approach assumes that all inputs during exposure are labeled as intended by the experimenter.
...
m.pen_compensated <-
  fit_NIW_ideal_adaptor(
    staninput = make_staninput(
      ...
      group.unique = "Condition", 
      response = "Response", 
      center.observations = T, scale.observations = F, tau_scale = sd(d.exposure.for_ibbu$cog_compensated),
      model_type = "NIW_ideal_adaptor"),
    ...
    plot_expected_categorization(
      ...
      stat_summary(
        data = 
          ...
          mutate(
            group = Condition,
            token.cues = factor(signif(cog_compensated), levels = sort(unique(signif(cog_compensated)), decreasing = T))) %>%
          group_by(token.cues, group, ParticipantID) %>%
          summarise(p_cat = mean(Response == "SH")),
        fun.data = mean_cl_boot, 
        geom = "pointrange", position = position_dodge(.25))
