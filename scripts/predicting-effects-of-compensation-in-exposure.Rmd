...
```{r, fig.height=fig.base_height, fig.width=fig.base_width}
...
d.test.for_ibbu <-
  ...
  # (since we are not yet modeling the effect during test in the ideal adaptor)
...
```{r, fig.height=fig.base_height*2+1, fig.width=fig.base_width*3}
groups <- get_group_levels_from_stanfit(m.pen_ignored, include_prior = T)
group.colors <- 
  case_when(
...
# TO DO -- REPAIR MVBL plot_in_cue_space seems to assume that there are at least two cues
# Also rename the functions to plot_stanfit_NIW_ideal_adaptor (keep aliases) and check the same for related get_ functions.
p <- 
  plot_ibbu_stanfit_test_categorization(
    m.pen_ignored,
    untransform_cues = T,
    panel.group = T,
    ndraws = 1000, group.colors = group.colors, group.linetypes = group.lt) 
