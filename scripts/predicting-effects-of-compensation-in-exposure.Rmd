...
### Visualize design [TO DO]

```{r, fig.height=fig.base_height*10, fig.width=fig.base_width*3}
d.exposure %>%
  ...
  ggplot(aes(x = Trial, y = Condition.Exposure.LexicalLabel, fill = Item.Type, alpha = Item.Pen)) +
  geom_tile() +
  scale_fill_manual(values = c(color.filler, color.s, color.sh)) +
...
TO DO: why are there 10 test locations? I was expecting 6 since we're using only PIH test tokens (each test block should have the same 12 items, half of which are PIH). Did test items differ across experiments?
    ...
    plot_expected_categorization(
      ...
      stat_summary(
        data = 
          ...
          mutate(
            group = Condition,
            token = paste0(as.numeric(as.factor(cog_compensated)), "\n", cog_compensated)) %>%
          group_by(token, group, ParticipantID) %>%
